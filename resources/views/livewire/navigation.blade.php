      ...
      <div class="relative flex items-center justify-between h-16">
          ...
          <div class="hidden sm:block sm:ml-6">
            <div class="flex space-x-4">
              ...
              @foreach ($categories as $c)
                <a href="{{route('posts.category',$c)}}" class="bg-gray-900 text-white px-3 py-2 rounded-md text-sm font-medium" aria-current="page">{{$c->name}}</a>
              @endforeach
        ...
        @auth
            ...
            <div class="ml-3 relative" x-data="{ open:false }">
                ...
                @can('admin.home'){{-- directiva encargado de verficar si tenemos permiso --}}
    ...
    <div x-show="open" x-on:click.away="open = false" class="sm:hidden" id="mobile-menu">
      <div class="px-2 pt-2 pb-3 space-y-1">
        ...
        <a href="#" class="text-gray-300 hover:bg-gray-700 hover:text-white block px-3 py-2 rounded-md text-base font-medium">Calendar</a> --}}
            @foreach ($categories as $c)
            <a href="{{route('posts.category',$c)}}" class="text-gray-300 hover:bg-gray-700 hover:text-white block px-3 py-2 rounded-md text-base font-medium">{{$c->name}}</a>
            @endforeach
       </div>
    </div>
  </nav>
